...
Future<String> _getStateList() async {
  final response = await http.post("http://locateme.codewaymx.com/mostrarColonias.php", body: {    
  });
...
Future<String> _getStateList1() async {
  final response = await http.post("http://locateme.codewaymx.com/mostrarRoles.php", body: {    
  });
  var datauser = json.decode(response.body);
  setState(() {
...
setState(() {
  ...
  registrarColonia();
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: const Text('El correo ya está registrado, ingrese otro'),));
      correo.text='';
  }
});
...
http.post('http://locateme.codewaymx.com/guardarColonia.php', body: {
    'col': _myState,
    'email' : correo.text,
  });
  setState(() {
 ...
 void Roles() {
   http.post("http://locateme.codewaymx.com/registrarRoles.php", body: {   
     'rol': _myState1,
    'email': correo.text,
  });
setState(() {
  print('4. registra el rol');
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: const Text('Se registró correctamente'),action: SnackBarAction(
          label: 'Iniciar sesión', onPressed: () {
...
requestPerms()async{
  ...
  Widget _loginForm(BuildContext context){
    ...
    Text('Registrate',style: TextStyle(fontSize: 30,foreground: Paint()
   ...
   Widget _apellidoDos(){
    return Container(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          icon: Icon(Icons.supervised_user_circle_sharp, color: Colors.blueAccent),
